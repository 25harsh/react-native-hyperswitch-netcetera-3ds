...
class HyperswitchNetcetera3ds: NSObject {
    ...
    func initialiseNetceteraSDK(_ apiKey: String,
                                ...
                                _ challengeCustomisationObject : String,
                                _ callback: @escaping RCTResponseSenderBlock) {
        var initStatus: [String:Any] = [:];

        let customizationMap = getChallengeUICustomisation(challengeCustomisationObject: challengeCustomisationObject)
        let locale = getChallengeLocale(challengeCustomisationObject: challengeCustomisationObject)
        do {
